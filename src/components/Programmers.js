...
export default function Programmers() {
  ...
  let [programmers, setProgrammers] = useState(listOfAwesome);
  let [progId, setProgId] = useState(null);
  ...
  const getNameOfFeatured = () => {
    ...
    return programmers[setProgId - 1]
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: progId  ? 'gold' : 'royalblue' // 🤔 color turns to gold, when celebrating
  };
  ...
  return (
    <div className='widget-programmers container'>
      ...
      <div className='programmers'>
        {
          ...
          programmers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => {setProgId(progId)}}>Feature</button>
            </div>
          )
        }
      </div>
      <div id='featured' style={style}>
        ...
        {(progId >= 0) ? `🎉 Let's celebrate ${getNameOfFeatured()}! 🥳`: 'Pick an awesome programmer'}
      </div>
    </div>
  );
}
